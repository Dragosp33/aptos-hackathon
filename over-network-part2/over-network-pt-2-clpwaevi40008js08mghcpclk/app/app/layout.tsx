...
export default async function RootLayout({
  ...
  */
  ...
  const setUpProfile = async (form: FormData) => {
    ...
    */
    //console.log(form)
    const newUser: Omit<User, 'followers' | 'following'> = {
    username: form.get('username')?.toString() || '', // If null, fallback to an empty string
    name: form.get('name')?.toString() || '', 
    privateKey: newPrivateKey(),
    };
    
    if (!newUser.username.trim() || !newUser.name.trim()) {
    throw new Error('Username and name cannot be empty');
    }
